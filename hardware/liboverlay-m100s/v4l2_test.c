                        ...
                        uint32_t global_alpha, char* img1, char *img2, uint32_t sleep_time, uint32_t num_frames)
{
    ...
    int num_bufs = 6, i, k, ret, size, fd;
    ...
    if ( v4l2_overlay_set_position(fd, 64, 64, LCD_WIDTH-128, LCD_HEIGHT-128) != 0 )
    {
    ...
    if ( v4l2_overlay_set_position(fd, 64, 64, LCD_HEIGHT-128, LCD_WIDTH-128) != 0 )     //  SWAP 
    {
        printf("Set Position Failed!\n");
        goto EXIT;
    }
...
#else
// frame 0
    memset (buf_info[0].start, 0x00, size/2);
    memset ((buf_info[0].start + size/2), 0xFF, size/2);

    memset (buf_info[1].start, 0xFF, size/2);
    memset ((buf_info[1].start + size/2), 0x00, size/2);

    memset (buf_info[2].start, 0x7F, size/2);
    memset ((buf_info[2].start + size/2), 0x00, size/2);

    memset (buf_info[3].start, 0xFF, size/2);
    memset ((buf_info[3].start + size/2), 0x7F, size/2);

    memset (buf_info[4].start, 0x7F, size/2);
    memset ((buf_info[4].start + size/2), 0xFF, size/2);

    memset (buf_info[5].start, 0x7F, size/2);
    memset ((buf_info[5].start + size/2), 0x7F, size/2);
...
int main (int argc, char *argv[])
{
    ...
    if (argc != 10) {
        ...
        printf("\n20 -\tOVERLAY_FORMAT_YCbCYr_422_I");
        return 0;    
    }
