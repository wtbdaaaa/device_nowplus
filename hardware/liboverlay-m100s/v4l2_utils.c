...
int v4l2_overlay_open(int id)
{
    ...
    {
        LOGV("aj v4l2_overlay_open V4L2_OVERLAY_PLANE_VIDEO1");
        return open("/dev/video1", O_RDWR);
    }
    else if (id == V4L2_OVERLAY_PLANE_VIDEO2)
    {  
        LOGV("aj v4l2_overlay_open V4L2_OVERLAY_PLANE_VIDEO2");
        return open("/dev/video2", O_RDWR);
    }
    return -EINVAL;
}

int v4l2_overlay_close(int id)
{
    LOG_FUNCTION_NAME
    return close(id);
}
...
int v4l2_overlay_init(int fd, uint32_t w, uint32_t h, uint32_t fmt)
{
    ...
    {
        LOGV("VIDIOC_QUERYCAP indicated that driver is not capable of Streaming");
        return -EINVAL;
    }
    ...
    //720p[[
    if((format.fmt.pix.width * format.fmt.pix.height)   == (1280 * 720)) 
    {
        is720p=1;
    }
    else
...
int v4l2_overlay_set_position(int fd, int32_t x, int32_t y, int32_t w, int32_t h)
{
    ...
    if(is720p ==1)
#else
    if((is720p ==1) && (isCamera == 0))   //VIK_RSZ_720
#endif
    {
        w=800;
        h=480;
    ...
    LOGI("v4l2_overlay_set_position:: left=%d top=%d w=%d h=%d",format.fmt.win.w.left,format.fmt.win.w.top, format.fmt.win.w.width, format.fmt.win.w.height);

    configure_window(&format.fmt.win, w, h, x, y);
    LOGV("v4l2_overlay_set_position_before ioctl:: w=%d h=%d", format.fmt.win.w.width, format.fmt.win.w.height);
    format.type = V4L2_BUF_TYPE_VIDEO_OVERLAY;
...
/*selwin changes: begin*/
int v4l2_overlay_set_mirroring(int fd, int degree, int step )
{
    LOG_FUNCTION_NAME
    int ret;
    struct v4l2_control control;
    ...
    ret = ioctl (fd, VIDIOC_S_CTRL, &control);
    if (ret < 0){
        error(fd, "VIDIOC_G_CTRL id : V4L2_CID_MIRRORing ioctl");
        return ret;
    }
...
int v4l2_overlay_set_watering(int fd, int degree, int step )
{
    LOG_FUNCTION_NAME
    int ret;
    struct v4l2_control control;
...
int v4l2_overlay_set_colorkey(int fd,  int enable, int colorkey)
{
    ...
    LOGV("Setting color key");
    
    
    if (enable){
        LOGV("enable clor key settings");
        ret = ioctl (fd, VIDIOC_G_FBUF, &v4l2_fb);
        LOGV("enable clor key settings----------1");
        if (ret < 0) {
        ...
        LOGV("enable clor key settings----------2");
        if (ret < 0) {
        ...
        LOGV("enable clor key settings----------3");
        if (ret < 0) {
...
#else       
        v4l2_fb.flags = V4L2_FBUF_FLAG_LOCAL_ALPHA;
        ret = ioctl (fd, VIDIOC_S_FBUF, &v4l2_fb);
        LOGV("enable clor key settings----------4");
        if (ret < 0) {
            error (fd, "VIDIOC_S_FBUF,: V4L2_FBUF_FLAG_LOCAL_ALPHA ioctl");
            return ret;
        }        
#endif
    }
    else{
        LOGV("Disable color key!!!!!!!!!!!!!!!!!!!!!!!!!");
    }
    return 0;
}
...
int v4l2_overlay_req_buf(int fd, uint32_t *num_bufs, int cacheable_buffers, int maintain_coherency)
{
    ...
    reqbuf.reserved[1] = maintain_coherency;
    ...
    LOGI("%d buffers allocated %d requested", reqbuf.count, 6);
