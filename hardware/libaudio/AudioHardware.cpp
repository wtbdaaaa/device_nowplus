...
status_t AudioHardware::setMode(int mode)
{
    ...
    if (status == NO_ERROR) {
        ...
        if (mMode != AudioSystem::MODE_IN_CALL && mInCallAudioMode) {
            ...
            setOutputRoute(ROUTE_OUT_PLAYBACK, AudioSystem::DEVICE_OUT_EARPIECE);//me chanage 2012.04.26
...
status_t AudioHardware::setIncallPath_l(uint32_t device)
{
    ...
    if (mMode == AudioSystem::MODE_IN_CALL) {
        ...
        if (mMixer != NULL) {
             setInputRoute(mInputSource, device);
    ...
    if (mMixer != NULL)
    {
        ...
        switch(path)
        {
            ...
            case ROUTE_OUT_VOICECALL:
                ...
                twlAudioRoute = (char*)getVoiceRouteFromDevice(device);
                break;
...
const char *AudioHardware::getOutputRouteFromDevice(uint32_t device)
{
    ...
    case AudioSystem::DEVICE_OUT_WIRED_HEADSET:
       ...
       return "HP";
#else
       if (mMode == AudioSystem::MODE_RINGTONE) return "SPK_HP";
        else return "HP";
#endif
...
const char *AudioHardware::getMax9877ModeFromDevice(uint32_t device)
{
    ...
    return "INB -> HP";
#else
        if (mMode == AudioSystem::MODE_RINGTONE) return "INA -> SPK and HP";
        else return "INB -> HP";
#endif
    case (AudioSystem::DEVICE_OUT_SPEAKER|AudioSystem::DEVICE_OUT_WIRED_HEADPHONE):
    case (AudioSystem::DEVICE_OUT_SPEAKER|AudioSystem::DEVICE_OUT_WIRED_HEADSET):
...
LOGD("--------AudioHardware::AudioStreamOutALSA::close_l().");

    if (mMixer) {
#if 1 //me change 
    if (mHardware->mode() != AudioSystem::MODE_IN_CALL)//me ok
        {
#endif
LOGD("in MODE_RINGTONE amp enable off.-------------------------1");
       // powerdown MAX9877
        TRACE_DRIVER_IN(DRV_MIXER_GET)
        ...
        LOGD("in MODE_RINGTONE amp enable not off.-------------------------2");
        }
